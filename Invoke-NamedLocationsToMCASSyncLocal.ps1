...
$GraphCreds = Get-Credential -Message "Username: ApplicationID, password: Application Secret"
$tenantID = Read-Host -Prompt "Specify TenantID"
$MCASCreds = Get-Credential -Message 'Username: MCAS URL, password: MCAS API Key'

$namedlocations = Get-NamedLocations -clientsecret $GraphCreds.GetNetworkCredential().password -clientid $GraphCreds.username -tenantId $tenantID

[Array]$ipranges = Get-MCASCorpIPRanges -credential $MCASCreds
    ...
    if($ipranges.count -ne 0 -and $ipranges.Name.contains($named.displayName)){
        ...
        foreach($iprange in $named.ipranges){
            ...
            if(!$MCASIPRange.Subnets.OriginalString.Contains($iprange.cidrAddress)){
                ...
                $MCASIPRange = Update-MCASIPRange -MCASIPRange $MCASIPRange -Subnet $iprange.cidrAddress -Add -credential $MCASCreds
            }
        }
        ...
        foreach($MCASsubnet in $MCASIPRange.subnets){
            if(!$named.ipranges.cidrAddress.contains($MCASsubnet.originalString)){
                ...
                $MCASIPRange = Update-MCASIPRange -MCASIPRange $MCASIPRange -Subnet $MCASsubnet -Remove -credential $MCASCreds
            }
        }
    }
    else {
        ...
        Create-MCASIPRange -DisplayName $named.displayName -IPRanges $named.ipRanges.cidrAddress  -credential $MCASCreds
    }
}
...
foreach($iprange in $ipranges){
    ...
    if(!$namedlocations.DisplayName.Contains($iprange.Name)){
        ...
        Remove-MCASIPRange -id $iprange._id -credential $MCASCreds
    }
}